...
enum class DType : uint8_t { FP32, FP16, BF16, Q115, Q131 };
...
#ifdef ENABLE_Q115
template <> __device__ float cast_value<float, int16_t>(int16_t val) {
  // Q1.15 to float conversion
  return __int2float_rn(val) / 32768.0f;
}

template <> __device__ int16_t cast_value<int16_t, float>(float val) {
...
int cudaMallocConditionallyManaged(void **out, size_t bytes, const char *file,
                                   int line) {
  // try to allocate
  cudaError_t err = cudaMalloc(out, bytes);
  if (err == cudaErrorMemoryAllocation) {
    // if we OOM, fallback to a managed allocation. slower but at least won't
    // crash.
...
// Use the preprocessor to handle both old and new API
    ...
    // Since cudaMemLocation mapping is problematic on some environments or
    // conditionally compiled out We just skip advise for the fallback managed
    // memory on CUDA 12+
#else
    cudaCheck_(cudaMemAdvise(*out, bytes, cudaMemAdviseSetPreferredLocation,
                             cudaCpuDeviceId),
               file, line);
#endif
    return 1;
...
__device__ __host__ constexpr unsigned int
SquirrelNoise5(unsigned int positionX, unsigned int seed) {
...
__device__ __host__ constexpr unsigned int
Get2dNoiseUint(int indexX, int indexY, unsigned int seed) {
  constexpr unsigned int PRIME_NUMBER =
      198491317u; // Large prime number with non-boring bits
...
stochastic_rounding(float in, __nv_bfloat16 *out, unsigned int seed) {
  // todo - is this stochastic rounding *too good*? can we cut any corners?
  // makes sure each thread gets a different random number
  unsigned int random =
      Get2dNoiseUint(threadIdx.x, blockIdx.x * blockDim.x + blockIdx.y, seed);
...
#ifdef ENABLE_Q115
__device__ __forceinline__ void stochastic_rounding(float in, int16_t *out,
                                                    unsigned int seed) {
