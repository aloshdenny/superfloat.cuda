...
#include "rand.h"
#include "utils.h"
#include <cuda_runtime.h> // for cudaMallocHost / cudaFreeHost (pinned memory)
  ...
  do {
    if (!(find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
      ...
      if (pathc >= pathv_capacity) {
        pathv_capacity *= 2;
        char **new_pathv =
            (char **)realloc(pathv, pathv_capacity * sizeof(char *));
        if (!new_pathv) {
...
typedef struct {
  ...
  size_t shard_num_samples; // total number of samples in the current shard per
                            // process
  // shards and current position
  glob_t glob_result; // stores the result of glob, for all shards we want to
                      // iterate
...
int64_t dataloader_load_shard_(DataLoader *loader, int shard_index) {
  ...
  // open the input file for reading. also only a single file can be opened at a
  // time
  ...
  assert(
      ntok >
      0); // we expect some tokens in the file. this should never trip, right?
  // determine the file size and make sure it is consistent with the number of
  // tokens
  fseekCheck(loader->tokens_file, 0, SEEK_END); // seek to end of file
  loader->file_size_bytes =
      ftell(loader->tokens_file); // read the offset, i.e. file size
...
void prepare_intra_shard_indices_(DataLoader *loader) {
  ...
  init_identity_permutation(loader->intra_shard_indices,
                            (int)loader->shard_num_samples);
  random_permutation(loader->intra_shard_indices,
                     (int)loader->shard_num_samples, &loader->shuffle_rng);
}
...
void dataloader_reset(DataLoader *loader) {
  ...
  if (loader->should_shuffle) { // shuffle the shards
    random_permutation(loader->shard_indices, (int)loader->glob_result.gl_pathc,
                       &loader->shuffle_rng);
  }
...
void dataloader_advance_(DataLoader *loader) {
  ...
  // advance the loader by loading the next data shard and resetting the
  // position
...
void dataloader_init(DataLoader *loader, const char *filename_pattern, size_t B,
                     size_t T, int process_rank, int num_processes,
                     int should_shuffle) {
  ...
  if (should_shuffle) {
    ...
    init_identity_permutation(loader->shard_indices,
                              (int)loader->glob_result.gl_pathc);
    loader->intra_shard_indices =
        NULL; // dynamically allocated allowing different shard sizes
  }
  ...
  for (int shard_index = 0; shard_index < loader->glob_result.gl_pathc;
       shard_index++) {
  ...
  cudaMallocHost((void **)&loader->targets, B * T * sizeof(int));
...
void dataloader_load_batch(DataLoader *loader) {
  assert(!loader->should_shuffle ||
         (loader->should_shuffle && loader->intra_shard_indices != NULL));
  assert(loader->current_sample_idx < loader->shard_num_samples);
  size_t idx = loader->should_shuffle
                   ? loader->intra_shard_indices[loader->current_sample_idx]
                   : loader->current_sample_idx;
  size_t global_batch_offset_bytes = idx * loader->total_batch_size_bytes;
  int64_t current_offset = loader->header_bytes + global_batch_offset_bytes +
                           loader->local_batch_offset_bytes;
...
void dataloader_resume(DataLoader *loader, size_t current_shard_idx,
...
- The examples follow, each example is a stream of uint16_t:
    ...
    - <EXAMPLE_BYTES>, bytes encoding this example, allowing efficient skip to
next
...
typedef struct {
  ...
  size_t
      B; // (micro) batch size dimension of the tensor that feeds into the model
...
void evalloader_reset(EvalLoader *loader) {
  ...
  int examples_per_process =
      CEIL_DIV(loader->num_examples, loader->num_processes);
  int can_fit_examples = (int)(loader->B / ASSUMED_NUM_COMPLETIONS);
  if (can_fit_examples == 0) {
    // this could be fixed in the future, but for now keeping it simple and
    // throw error when B too low
    printf("HellaSwag EvalLoader: batch size %zu is < %d\n", loader->B,
           ASSUMED_NUM_COMPLETIONS);
    printf("---> HINT: Disable HellaSwag eval with -h 0, or increase batch "
           "size with -b\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < loader->start_example_index; i++) {
    ...
    assert(example_header[2] ==
           i); // <EXAMPLE_INDEX> should match the loop index
  ...
  // now we are at the start of the example we want to start at, pointing at
  // <START_EXAMPLE>
  loader->current_example_index = loader->start_example_index;
}

void evalloader_init(EvalLoader *loader, const char *filename, size_t B,
                     size_t T, int process_rank, int num_processes) {
  loader->process_rank = process_rank;
  loader->num_processes = num_processes;
  loader->B = B;
  ...
  // contains the prompt (or "context") and 4 completions, all of these have to
  // be up to T tokens, and their tokens are uint16_t (so 2 bytes/token).
  // There's a few more things in each example but they are minor.
  // So longest example should be roughly this. Just trying to make sure it's
  // sensible.
  assert(longest_example_bytes > 0 &&
         longest_example_bytes < (1 + ASSUMED_NUM_COMPLETIONS) * T * 2);
...
void evalloader_next_example_(EvalLoader *loader, int example_batch_index) {
  // this function populates the inputs, targets, mask, and label fields for one
  // example because every (B,T) tensor can fit multiple examples and we want to
  // take advantage, we also pass in the example_batch_index to indicate which
  // example in the batch we are loading and each example takes up
  // ASSUMED_NUM_COMPLETIONS rows in the batch
  size_t B = loader->B;
  size_t T = loader->T;
  ...
  // validate the <EXAMPLE_INDEX>
  assert(example_header[2] ==
         loader->current_example_index); // <EXAMPLE_INDEX> should match the
                                         // loop index
  assert(example_header[2] >= loader->start_example_index &&
         example_header[2] < loader->end_example_index);
  // read the rest of the example (we have space for 3 more uint16_t values in
  // buffer, it's ok)
  size_t example_bytes = example_header[1] - sizeof(uint16_t) * 3;
  // read example_bytes into buffer. careful that this is actually in the units
  // of bytes
  ...
  int can_fit_examples = (int)(loader->B / ASSUMED_NUM_COMPLETIONS);
  assert(label >= 0 &&
         label < ASSUMED_NUM_COMPLETIONS); // we expect the label to be in [0,
                                           // 4) for right now
  ...
  assert(num_completions ==
         ASSUMED_NUM_COMPLETIONS); // we expect 4 completions for now
  assert(batch_dim_offset + num_completions <=
         B);                                 // we expect to fit in the batch
  loader->num_completions = num_completions; // store for output
  // process the context
  // the context is shared for all completions, so we insert it into all data
  // rows equally
  int context_length = (int)loader->buffer[2];
  uint16_t *context_tokens_start = &loader->buffer[3]; // where the tokens start
  assert(context_length > 0 &&
         context_length < T); // context is non-empty and up to T
  ...
  // process the completions, insert them in their row, right after the (shared)
  // context
  ...
  for (int c = 0; c < num_completions; c++) {
    ...
    assert(completion_length > 0 &&
           context_length + completion_length < T); // things fit?
...
void evalloader_next_batch(EvalLoader *loader) {
  size_t B = loader->B;
  size_t T = loader->T;
  // init mask to zeros, no need to do it for inputs & targets, the values where
  // the mask is set will be correctly overwritten every time.
  ...
  int can_fit_examples =
      (int)(B / ASSUMED_NUM_COMPLETIONS); // how many examples can we fit in the
                                          // batch?
