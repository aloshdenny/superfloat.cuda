  ...
  if (weight_init) {
    ...
    if (PRECISION_MODE == PRECISION_FP32 && version != 3) {
      ...
      fprintf(stderr, "---> HINT: to turn on FP32 you have to compile like: "
                      "`make train_gpt2rawcu PRECISION=FP32`\n");
...
int main(int argc, char *argv[]) {
  ...
  const char *default_load_filename =
      "gpt2_124M_bf16.bin"; // bf16 weights of the model
  ...
  if (resuming == 1) {
    ...
    // If the default checkpoint isn't present, fall back to random init via
    // descriptor. This keeps the program runnable out-of-the-box while still
    // allowing `-e` to point at a real checkpoint.
    if (strcmp(load_filename, default_load_filename) == 0 &&
        access(load_filename, F_OK) != 0) {
