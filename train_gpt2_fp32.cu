...
void gpt2_forward(GPT2 *model, int *inputs, int *targets, int B, int T) {
  ...
  if (targets != NULL) {
    // fused classifier: does the forward pass and first part of the backward
    // pass we're passing dlosses = NULL, which will default them to 1.0f/(B*T),
    // i.e. uniform loss
    fused_classifier3(acts.output, acts.losses, NULL, model->targets, B, T, V,
                      Vp);
    // for convenience also evaluate the mean loss (TODO re-think this
    // compute+sync point) move the (B,T) losses to CPU
...
void gpt2_backward(GPT2 *model) {
  ...
  float *residual =
      acts.residual3 + (L - 1) * B * T * C; // last residual is in residual3
  float *dresidual = grads_acts.residual3;  // the main buffer holding the
  ...
  for (int l = L - 1; l >= 0; l--) {
    ...
    // we need a B x T x C buffer; thankfully, the forward activation for lnf
    // isn't needed anymore, so we can co-opt it here.
    ...
    matmul_backward(dl_btc, dl_attprojw, dl_attprojb, dresidual, l_atty,
                    l_attprojw, B, T, C, C);
    // we more B x T x (4)C buffers. l_atty and l_fch aren't needed anymore at
    // this point, so reuse their memory
    float *buffer_a = l_atty;
    float *buffer_b =
        l_fch; // this is B x T x 4C, so even larger than what we need
