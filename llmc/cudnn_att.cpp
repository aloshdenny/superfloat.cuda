// all cudnn-related functions are in this file, so that they don't need to be
// recompiled everytime we change some unrelated piece of the code.
...
static size_t cudnn_workspace_size =
    0; // dynamically allocated as needed (up to 256MiB!)
...
static void checkCudnnFE(const fe::error_object &e, const char *file,
                         int line) {
  if (!e.is_good()) {
...
// Need a cache because graph->build_operation_graph() is slow but everything
// else seems fast
using cache_type_fwd = std::map<std::tuple<int, int, int, int, int>,
                                std::shared_ptr<fe::graph::Graph>>;
using cache_type_bwd =
    std::map<std::tuple<int, int, int, int>, std::shared_ptr<fe::graph::Graph>>;

// Loosely based on cuDNN frontend samples functions and massively simplified
auto lookup_cache_or_build_graph_fwd(int B, int H, int T, int HS,
  ...
  // QKV is (B, T, 3, NH, HS) which cuDNN can handle directly without an
  // external permute
  ...
  // Output is (B, T, NH, HS) BF16/FP16 and stats for backward pass is (B, NH,
  // T) FP32
  O->set_output(true)
      .set_dim({B, H, T, HS})
  ...
  // Reallocate the workspace if the required size is greater than the current
  // workspace In H100 this may be around 16B
...
auto lookup_cache_or_build_graph_bwd(int B, int NH, int T, int HS) {
  ...
  // Reallocate the workspace if the required size is greater than the current
  // workspace By default, cuDNN uses up to 256MiB of workspace, so we don't
  // want to just allocate the maximum
  if (graph->get_workspace_size() > cudnn_workspace_size) {
    if (cudnn_workspace_size > 0) {
      cudaCheck(cudaFree(cudnn_workspace));
    }
...
void attention_forward_cudnn(
  ...
  // Add the stats tensor unless we are only doing inference (only needed for
  // backward pass)
  if (is_inference_only == false) {
...
void attention_backward_cudnn(floatX *dqkvr, // output
                              floatX *dout, floatX *qkvr, floatX *o,
                              float *stats, // inputs
  ...
  std::unordered_map<int64_t, void *> variant_pack = {
      {Q_UID, devPtrQ},   {K_UID, devPtrK},
      {V_UID, devPtrV},   {O_UID, devPtrO},
      {dO_UID, devPtrdO}, {Stats_UID, devPtrStats},
      {dQ_UID, devPtrdQ}, {dK_UID, devPtrdK},
      {dV_UID, devPtrdV}, {Attn_scale_UID, &attn_scale_cpu}};
