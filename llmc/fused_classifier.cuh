...
- (fusion) Also kicks off the backward pass, because everything is already
loaded
*/
...
// Q1.15 logit scaling: Q1.15 stores values in [-1, 1), but logits need [-24,
...
// up by Q115_LOGIT_SCALE before computing softmax for proper probability
// distribution Gradients are computed in float and then stored back with
// scaling for Q1.15 storage
...
__global__ void __launch_bounds__(1024, MAX_1024_THREADS_BLOCKS)
  ...
  for (int i = threadIdx.x; i < V; i += blockDim.x) {
    // Logits stored as raw BF16; just scale to expand effective range.
    float logit = (float)logits_row[i] * logit_scale;
  ...
  if (threadIdx.x == 0) {
    ...
    for (int w = 0; w < blockDim.x / WARP_SIZE; w++) {
      // This is a simplification - real implementation would use proper float
      // atomics
    }
  }
  ...
  if (threadIdx.x == 0) {
    float target_logit = (float)logits_row[target_ix] * logit_scale;
  ...
  if (WriteDLogits) {
    ...
    for (int i = threadIdx.x; i < V; i += blockDim.x) {
      ...
      // Store gradient back to logits buffer (raw BF16, no re-quantization)
      logits[idx * P + i] = (floatX)grad_scaled;
    }
...
__device__ SoftmaxParams prepare_softmax_blockwide3(int64_t idx,
  ...
  while ((i + 1) * x128::size > V) {
    for (int k = 0; k < x128::size; ++k) {
      ...
      // Logits are raw BF16; no re-quantization to Q1.15 needed here.
...
__global__ void __launch_bounds__(1024, MAX_1024_THREADS_BLOCKS)
  ...
  // by making it a long here, we ensure that any offsets calculated with it
  // (e.g., idx * P) are done is 64 bit
  int64_t idx = gridDim.x -
  ...
  // the logits used above to compute the loss are concurrently (race) modified
  // to carry backward pass grads. since the "logits" are overwritten to be in
  // the [-1, 1] range and sp.Offset is sometimes smaller than -90 we
  // errouneously end up computing exp^(90+) which gives us infinities in the
  // loss! this is the fix.
  __syncthreads();
...
void fused_classifier(Type *logits, float *losses, const float dloss,
  ...
  // softmax
  q115_scaled_softmax_ce_kernel<<<grid_size, block_size, 0, stream>>>(
      logits, losses, dloss, targets, B, T, V, P, write_dlogits);
#else
  fused_classifier_kernel5<<<grid_size, block_size, 0, stream>>>(
      logits, losses, (floatX *)NULL, dloss, targets, B, T, V, P,
      write_dlogits);
